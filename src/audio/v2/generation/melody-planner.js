...
const MELODY_STYLE = {
    default: { density: 0.5, leap: 0.48, velocity: 0.62, highOct: 0.35, micro: 0.4 },
    ethereal: { density: 0.32, leap: 0.22, velocity: 0.44, highOct: 0.62, micro: 0.62 },
    nebula: { density: 0.3, leap: 0.24, velocity: 0.46, highOct: 0.66, micro: 0.65 },
    glacial: { density: 0.24, leap: 0.2, velocity: 0.38, highOct: 0.72, micro: 0.52 },
    arctic: { density: 0.26, leap: 0.22, velocity: 0.4, highOct: 0.7, micro: 0.5 },
    barren: { density: 0.16, leap: 0.12, velocity: 0.32, highOct: 0.52, micro: 0.25 },
    oceanic: { density: 0.38, leap: 0.28, velocity: 0.48, highOct: 0.5, micro: 0.45 },
    crystalline: { density: 0.36, leap: 0.28, velocity: 0.5, highOct: 0.58, micro: 0.55 },
    storm: { density: 0.44, leap: 0.44, velocity: 0.62, highOct: 0.26, micro: 0.35 },
    corrupted: { density: 0.42, leap: 0.46, velocity: 0.6, highOct: 0.34, micro: 0.58 },
    quantum: { density: 0.4, leap: 0.48, velocity: 0.6, highOct: 0.48, micro: 0.78 },
    volcanic: { density: 0.36, leap: 0.38, velocity: 0.58, highOct: 0.16, micro: 0.3 },
    fungal: { density: 0.58, leap: 0.44, velocity: 0.62, highOct: 0.34, micro: 0.4 },
    abyssal: { density: 0.28, leap: 0.3, velocity: 0.52, highOct: 0.14, micro: 0.36 },
};

const BIOME_VOICE_HINTS = {
    ethereal: {
        ambient: ['drone_morph', 'vowel_morph', 'choir'],
        motif: ['vowel_morph', 'drone_morph'],
    ...
    glacial: {
        ambient: ['granular_cloud', 'drone_morph', 'crystal_chimes'],
        motif: ['crystal_chimes', 'granular_cloud'],
        intense: ['modal_resonator'],
    },
    storm: {
        ambient: ['drone_morph', 'wavetable_morph'],
    ...
    abyssal: {
        ambient: ['drone_morph', 'gong', 'subpad'],
        motif: ['modal_resonator', 'gong'],
        intense: ['modal_resonator', 'gong'],
    },
    default: {
        ambient: ['drone_morph', 'wavetable_morph', 'vowel_morph'],
...
export class MelodyPlanner {
    ...
    planStep({
        ...
        const phraseBase = 4 + Math.round((motion + complexity) * 4 + this.moonDensity * 2.4);
        const phraseLen = stretchRhythm(
            phraseBase,
            section === 'AFTERGLOW' ? 1.35 : section === 'SURGE' ? 0.9 : 1,
        );
        ...
        if (phrasePos === 0) {
            ...
            const swapEvery = clamp(Math.round(2 + (1 - this.style.density) * 3 - complexity * 1.2), 1, 5);
        ...
        const styleDensity = this.style.density * (0.7 + this.melodyDensity * 1.6);
        let playChance = clamp(
            (0.2 + complexity * 0.22 + motion * 0.16 + tension * 0.08 - (1 - qualityScalar) * 0.28) * styleDensity,
            0.05,
            0.9,
        );
        if (isPhraseEnd) playChance *= 0.88;
        if (section === 'AFTERGLOW') playChance *= 0.82;
        if (section === 'SURGE') playChance *= 1.1;
        ...
        if (shouldPlay) {
            const motifChance = clamp(
                (isResponse ? 0.34 : 0.14) + this.style.density * 0.16 + complexity * 0.12 + this.moonDensity * 0.08,
                0.06,
                0.8,
            );
            const responseChance = clamp(
                (isResponse ? 0.28 : 0.06) + motion * 0.14 + this.style.leap * 0.08 + this.moonDensity * 0.06,
                0.04,
                0.62,
            );

            if (this.motifBank.length && rng.bool(motifChance)) {
                mode = 'MOTIF';
                ...
                const variation = rng.pick([-2, -1, 0, 1, 2]);
        ...
        const microChance = clamp((0.08 + dissonance * 0.16 + this.quarterToneProb * 0.2) * this.style.micro, 0.04, 0.62);
        const microCents = shouldPlay && rng.bool(microChance)
            ? (this.quarterToneProb > 0 && rng.bool(this.quarterToneProb * 0.58)
                ? rng.pick([-50, 50]) * clamp(0.5 + dissonance * 0.55, 0.45, 1)
                : rng.range(-22, 22))
            : 0;

        let counterline = null;
        if (counterlineEnabled && qualityScalar > 0.6 && rng.bool(0.08 + complexity * 0.12 + this.moonDensity * 0.05)) {
            const idx = this.scale.indexOf(this.lastCounterStep);
            const safeIdx = idx >= 0 ? idx : 0;
            const movePool = section === 'AFTERGLOW' ? [-2, -1, 1] : [-2, -1, 1, 2];
            const nextIdx = ((safeIdx + rng.pick(movePool)) % this.scale.length + this.scale.length) % this.scale.length;
            this.lastCounterStep = this.scale[nextIdx] + (rng.bool(0.14) ? 12 : 0);
            counterline = this.lastCounterStep;
        }

        const voiceHint = this._pickVoiceHint(rng, { mode, section, dissonance, complexity });
        const octave = this._pickOctave(rng, { section, mode });
        let velocity = clamp((0.18 + complexity * 0.3 + motion * 0.14 + (isPhraseEnd ? 0.05 : 0)) * this.style.velocity, 0.08, 0.84);
        if (section === 'AFTERGLOW') velocity *= 0.88;
        if (!shouldPlay) velocity = 0;
        let durScale = section === 'AFTERGLOW' ? 1.45 : section === 'SURGE' ? 0.92 : 1.04;
    ...
    _buildMotif(rng, complexity, dissonance, section, motifIdx) {
        ...
        if (section === 'SURGE' && rng.bool(0.12 + complexity * 0.14)) motif = rotate(motif, 2);
    ...
    _chooseGenerativeStep(rng, {
        ...
        this.scale.forEach((scaleStep) => {
            ...
            if (chordTone) {
                ...
                weight *= clamp(1 - this.chordAudibility * 0.55 + dissonance * 0.22, 0.34, 1.42);
            }

            const tonicOrFifth = norm === 0 || norm === 7;
            if (isPhraseEnd || isResponse) {
                weight *= tonicOrFifth || chordTone ? 2.15 : 0.7;
            } else {
                weight *= !chordTone ? (1.02 + dissonance * 0.14) : 1;
            }
            ...
            if (Number.isFinite(this.lastStep)) {
                ...
                else if (diff >= 9) weight *= 0.5 + complexity * 0.2;
            }

            if (section === 'AFTERGLOW' && !chordTone) weight *= 0.72;
            if (section === 'SURGE' && !chordTone) weight *= 1.06 + dissonance * 0.1;
            if (section === 'INTRO' && phrasePos < 2 && !tonicOrFifth) weight *= 0.82;
            if (this.biomeId === 'fungal' && norm === 0 && !isPhraseEnd) weight *= 0.78;
            if (chordSymbol === 'V' && norm === 11) weight *= 1.2;

            const copies = clamp(Math.round(weight * 4), 1, 14);
            for (let i = 0; i < copies; i++) pool.push(scaleStep);
        });

        let step = pool.length ? rng.pick(pool) : this.scale[0];
        const leapChance = clamp((0.05 + complexity * 0.17 + dissonance * 0.1) * this.style.leap, 0.02, 0.5);
        if (rng.bool(leapChance)) {
            step = shiftScaleDegree(this.scale, step, rng.pick([3, -3, 4, -4]));
        } else if (rng.bool(0.24 + this.moonDensity * 0.1)) {
            step = shiftScaleDegree(this.scale, step, rng.pick([0, 1, -1, 2, -2]));
        }
        return step;
    }

    _pickVoiceHint(rng, { mode, section, dissonance, complexity }) {
        const hints = BIOME_VOICE_HINTS[this.biomeId] || BIOME_VOICE_HINTS.default;
        if (section === 'AFTERGLOW') return rng.pick(hints.ambient);
        if (mode === 'MOTIF' && rng.bool(0.58)) return rng.pick(hints.motif);
        if (section === 'SURGE' && (dissonance > 0.68 || complexity > 0.72)) {
            if (this.softBiome) return rng.pick(['wavetable_morph', 'drone_morph']);
            return rng.pick(hints.intense);
        }
        if (mode === 'RESPONSE' && rng.bool(0.4)) {
            return this.softBiome ? rng.pick(['vowel_morph', 'drone_morph']) : rng.pick(['modal_resonator', 'wavetable_morph']);
        }
        return null;
    }

    _pickOctave(rng, { section, mode }) {
